...
class SnakeEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, size=20):
        ...
        self.observation_space = spaces.Dict({
                "snake": spaces.Box(0, size - 1, shape=(self.snake.get_length(), 2), dtype=int),
    ...
    def _get_obs(self):
        return {"snake": np.array(self.snake.pos), "apple": self.apple}
    ...
    def step(self, action):
        ...
        if (np.array(curr_head_pos) == np.array(self.apple)).all():
